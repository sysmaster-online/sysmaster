...
`devmaster`提供了两种策略设置方法：

1. 配置启动参数: `net.naming-scheme=<scheme>`

2. 设置环境变量: `NET_NAMING_SCHEME=<scheme>`

环境变量`NET_NAMING_SCHEME`的值以':'开头时，优先使用启动参数中指定的策略，否则优先使用环境变量中指定的策略。命名策略主要影响内置命令`net_id`获取的网卡属性，同时会改变`net_setup_link`或规则处理过程中的网卡命名行为。设置为`v000`或`0`关闭网卡重命名规则。
