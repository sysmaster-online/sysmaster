...
struct UnitObjWrapper(Rc<UnitX>);

impl Deref for UnitObjWrapper {
    type Target = Rc<UnitX>;
...
impl UeDep {
    ...
    pub fn get(&self, relation: UnitRelations) -> Vec<Rc<UnitX>> {
    ...
    fn getDependencies(&self) -> Vec<(UnitRelations, Rc<UnitX>)> {
        let mut v_dependencies: Vec<(UnitRelations, Rc<UnitX>)> = Vec::new();
