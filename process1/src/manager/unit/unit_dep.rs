...
impl UnitDep {
    ...
    pub(super) fn insert(&self, source:Rc<UnitX>, relation:UnitRelations, dest:Rc<UnitX>, source_mask:u16) {
        self.data.borrow_mut().insert(source, relation, dest, source_mask)
    }
    ...
    pub(super) fn is_dep_with(&self, source:&UnitX, relation:UnitRelations, dest:&UnitX) -> bool {
...
impl UnitDepData {
    ...
    pub(self) fn insert(&mut self, source:Rc<UnitX>, relation:UnitRelations, dest:Rc<UnitX>, source_mask:u16) {
        // check input
        if source.as_ref() == dest.as_ref() { // ptr_eq?
        ...
        self.insert_one_way(Rc::clone(&source), relation, Rc::clone(&dest), UnitDepMask{source:source_mask, dest:0,});
        self.insert_one_way(dest, relation_inverse, source, UnitDepMask{source:0, dest:source_mask,});
    }
    ...
    pub(self) fn get(&self, source:&UnitX, relation:UnitRelations) -> Vec<Rc<UnitX>> {
        ...
        if let Some(sv) = self.t.get(source) {
            if let Some(dv) = sv.get(&relation) {
                dests.append(&mut dv.iter().map(|(destr, _)| Rc::clone(destr)).collect::<Vec<_>>());
            }
        }
    ...
    fn insert_one_way(&mut self, source:Rc<UnitX>, relation:UnitRelations, dest:Rc<UnitX>, mask:UnitDepMask) {
    ...
    fn get_mut_sv_pad(&mut self, source:Rc<UnitX>) -> &mut HashMap<UnitRelations, HashMap<Rc<UnitX>, UnitDepMask>> {
        // verify existance
        if let None = self.t.get(&source) { // nothing exists, pad it.
            self.t.insert(Rc::clone(&source), HashMap::new());
        }

        // return the one that must exist
        self.t.get_mut(&source).expect("something inserted is not found.")
    }

    fn get_mut_dv_pad(&mut self, source:Rc<UnitX>, relation:UnitRelations) -> &mut HashMap<Rc<UnitX>, UnitDepMask> {
        // verify existance
        let sv = self.get_mut_sv_pad(source);
        if let None = sv.get(&relation) { // nothing exists, pad it.
            sv.insert(relation, HashMap::new());
        }

        // return the one that must exist
        sv.get_mut(&relation).expect("something inserted is not found.")
    }
}
