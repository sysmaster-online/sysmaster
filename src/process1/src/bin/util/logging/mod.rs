...
pub fn init_log_with_console(app_name:&str,log_level:u32){
    ...
    pattern += "{h({l}):<5} ";
    pattern += "{m}{n}";
    let stdout = ConsoleAppender::builder().encoder(Box::new(PatternEncoder::new(&pattern))).target(Target::Stderr).build();
    let logging_builder = Config::builder()
    ...
    let config = match Some(app_name){
        Some(a_p) => logging_builder.logger(Logger::builder().build(a_p, l_level)).build(Root::builder().appender("console").build(l_level)),
