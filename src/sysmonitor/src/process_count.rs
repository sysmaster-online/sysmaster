...
fn alarm_default() -> u32 { 1600 }

fn resume_default() -> u32 { 1500 }

fn period_default() -> u32 { 60 }

fn alarm_ratio_default() -> f32 { 90.0 }

fn resume_ratio_default() -> f32 { 80.0 }

fn show_top_proc_num_default() -> u32 { 10 }
...
impl Monitor for ProcessCount {
    ...
    fn is_valid(&self) -> bool {
        self.alarm > self.resume && self.resume > 0 && self.period > 0 && 0.0 < self.resume_ratio &&
            self.resume_ratio < self.alarm_ratio && self.alarm_ratio < 100.0 && self.show_top_proc_num < 1024
    }
    ...
    fn check_status(&mut self) -> Result<(), SysMonitorError> {
        ...
        if self.show_top_proc_num > 0 {
            thread_num = all_processes.iter().fold(0, |num, process| num + process.stat.num_threads as u32);
        }

        println!("{} {}", proc_num, thread_num);
        let pid_max = pid_max()?;
        if pid_max == 0 {
            return Err(SysMonitorError::ProcfsError(ProcError::from(Error::new(ErrorKind::Other, "found pid_max is 0"))));
        }
        let real_alarm = max(self.alarm, (pid_max as f32 * self.alarm_ratio / 100.0) as u32);
        let real_resume = max(self.resume, (pid_max as f32 * self.resume_ratio / 100.0) as u32);
        //  self.count = proc_num;
        if proc_num >= real_alarm && !self.status {} else if proc_num <= real_resume && self.status {}
