...
//!
//! 设置套接字的接受和发送缓冲区的大小， 选项可以以K、M、G等为单位。
//!
//! PassPacketInfo
//!
//! 可设为 on 或 off(默认), 是否获取接收报文的相关信息，也可在发送报文时指定报文的相关控制信息。
//!
//! PassCredentials
//!
//! 可设为 on 或 off(默认), 是否允许套接字接收对端进程在辅助消息中发送的证书。
//!
//! PassSecurity
//!
//! 可设为 on 或 off(默认), 是否允许套接字接收对端进程在辅助消息中发送的安全上下文。
//!
//! SocketMode
//!
