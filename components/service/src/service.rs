...
use process1::manager::unit::unit_entry::{ConfigParser, UnitObj, Unit, UnitActiveState, KillOperation};
...
impl Default for ServiceResult {
    fn default() -> Self {ServiceResult::ServiceResultInvalid}
}
...
impl Default for ServiceState {
    fn default() -> Self { ServiceState::ServiceStateMax}
}
...
impl ServiceState {
    fn to_unit_active_state(&self) -> UnitActiveState {
        match *self {
            ...
            ServiceState::ServiceCondition | ServiceState::ServiceStartPre |
                ServiceState::ServiceStart | ServiceState::ServiceStartPost => UnitActiveState::UnitActivating,
            ServiceState::ServiceRuning | ServiceState::ServiceExited => UnitActiveState::UnitActive,
                ServiceState::ServiceReload => UnitActiveState::UnitReloading,
            ServiceState::ServiceStop | ServiceState::ServiceStopWatchdog |
                ServiceState::ServiceStopPost | ServiceState::ServiceStopSigterm |
                ServiceState::ServiceStopSigkill | ServiceState::ServiceStateMax |
...
pub struct ServiceUnit {
    ...
    pub exec_commands: [LinkedList<Rc<RefCell<CommandLine>>>; ServiceCommand::ServiceCommandMax as usize],
}   
...
impl ServiceUnit {
    ...
    fn set_state(&mut self, m: &mut UnitManager, state: ServiceState) {
        ...
        log::debug!("original state: {:?}, change to: {:?}", original_state, state);
        // todo!()
        // trigger the unit the dependency trigger_by

        self.unit.notify(m, original_state.to_unit_active_state(), state.to_unit_active_state());
    }
        ...
        match cmd.next() {
            Some(cmd) => {
                ...
                match service_start::start_service(self, m, &*cmd.borrow()) {
                    ...
                    Err(_e) => {
                        log::error!("failed to start service: {}", self.unit.id.clone());
                        self.send_signal(m, ServiceState::ServiceStopSigterm, ServiceResult::ServiceFailureResources);
                    },
                }
    ...
    fn send_signal(&mut self, m: &mut UnitManager, state: ServiceState, res: ServiceResult) {
        log::debug!("Sending signalsend signal of state: {:?}, service result: {:?}", state, res);
        let operation = state.to_kill_operation();

        self.kill_service(operation);

        if vec![ServiceState::ServiceStopWatchdog, ServiceState::ServiceStopSigterm, ServiceState::ServiceStopSigkill].contains(&state) {
            self.run_stop_post(m, ServiceResult::ServiceSuccess);
        } else if vec![ServiceState::ServiceFinalWatchdog, ServiceState::ServiceFinalSigterm].contains(&state) {
            self.send_signal(m, ServiceState::ServiceFinalSigkill, ServiceResult::ServiceSuccess);
        } else {
            self.run_dead(m, ServiceResult::ServiceSuccess);
        }

        log::debug!("Sending signal, state: {:?}, service result: {:?}", state, res);
    }
    ...
    pub fn run_stop(&mut self, m: &mut UnitManager, res: ServiceResult) {
        ...
        match cmd.next() {
            ...
            None => {
                self.send_signal(m, ServiceState::ServiceStopSigterm, ServiceResult::ServiceSuccess);
            },
        }
    }
        ...
        match cmd.next() {
            Some(cmd) => {
                ...
                match service_start::start_service(self, m, &*cmd.borrow()) {
                    Ok(pid) => self.control_pid = Some(pid),
                    Err(_e) => {
                        self.send_signal(m, ServiceState::ServiceFinalSigterm, ServiceResult::ServiceFailureResources);
                        log::error!("Failed to run stop service: {}", self.unit.id.clone());
                    },
                }
            ...
            None => {
                self.send_signal(m, ServiceState::ServiceFinalSigterm, ServiceResult::ServiceSuccess);
            },
        }
    }
...
impl ServiceUnit {
    fn sigchld_event(&mut self, m: &mut UnitManager, pid: Pid, code: i32, status: Signal) {
        log::debug!("ServiceUnit sigchld exit, pid: {:?} code:{}, status:{}", pid, code, status);
        log::debug!("main_pid ：{:?}, control_pid: {:?}, state: {:?}", self.main_pid, self.control_pid, self.state);
        ...
        if self.main_pid == Some(pid) {
            ...
            if self.main_command.is_some() && self.main_command.as_ref().unwrap().borrow().next.is_some() &&
                res == ServiceResult::ServiceSuccess {
                ...
                match self.state {
                    ...
                    ServiceState::ServiceStopWatchdog | ServiceState::ServiceStopSigkill |
            ...
            if self.control_command.is_some() && self.control_command.as_ref().unwrap().borrow().next.is_some() &&
                res == ServiceResult::ServiceSuccess {
                ...
                match self.state {
                    ...
                    ServiceState::ServiceStopSigterm | ServiceState::ServiceStopSigkill | 
                        ServiceState::ServiceStopWatchdog => {
                            self.run_stop_post(m, res);
                        },
...
impl UnitObj for ServiceUnit {
    ...
    fn stop(&mut self, m: &mut UnitManager) {
        ...
        let stop_state = vec![ServiceState::ServiceStop, 
                                            ServiceState::ServiceStopSigterm,
        ...
        let starting_state = vec![ServiceState::ServiceCondition, 
                                            ServiceState::ServiceStartPre,
                                            ServiceState::ServiceStart,
                                            ServiceState::ServiceStartPost,
                                            ServiceState::ServiceReload,
...
impl ConfigParser for ServiceUnit {
    fn parse(&mut self, manager: &mut UnitManager)  -> Result<(), Box<dyn Error>> {
        ...
        match &service.exec_condition {
            None => {self.exec_commands[ServiceCommand::ServiceCondition as usize] = LinkedList::new();},
            Some(commands) => {
                match prepare_command(commands, &mut self.exec_commands[ServiceCommand::ServiceCondition as usize]){
                    Ok(_) => {},
                    Err(e) => return Err(e),
                }
            }
        ...
        match &service.exec_prestart {
            None => {self.exec_commands[ServiceCommand::ServiceStartPre as usize] = LinkedList::new();},
            Some(commands) => {
                match prepare_command(commands, &mut self.exec_commands[ServiceCommand::ServiceStartPre as usize]){
                    Ok(_) => {},
                    Err(e) => return Err(e),
                }
            }
        ...
        match &service.exec_start {
            ...
            Some(commands) => {
                match prepare_command(commands, &mut self.exec_commands[ServiceCommand::ServiceStart as usize]){
                    Ok(_) => {},
                    Err(e) => return Err(e),
                }
            }
        ...
        match &service.exec_startpost {
            None => {self.exec_commands[ServiceCommand::ServiceStartPost as usize] = LinkedList::new();},
            Some(commands) => {
                match prepare_command(commands, &mut self.exec_commands[ServiceCommand::ServiceStartPost as usize]){
                    Ok(_) => {},
                    Err(e) => return Err(e),
                }
            }
        ...
        match &service.exec_reload {
            None => {self.exec_commands[ServiceCommand::ServiceReload as usize] = LinkedList::new();},
            Some(commands) => {
                match prepare_command(commands, &mut self.exec_commands[ServiceCommand::ServiceReload as usize]){
                    Ok(_) => {},
                    Err(e) => return Err(e),
                }
            }
        ...
        match &service.exec_stop {
            None => {self.exec_commands[ServiceCommand::ServiceStop as usize] = LinkedList::new();},
            Some(commands) => {
                match prepare_command(commands, &mut self.exec_commands[ServiceCommand::ServiceStop as usize]){
                    Ok(_) => {},
                    Err(e) => return Err(e),
                }
            }
        ...
        match &service.exec_stoppost {
            None => {self.exec_commands[ServiceCommand::ServiceStopPost as usize] = LinkedList::new();},
            Some(commands) => {
                match prepare_command(commands, &mut self.exec_commands[ServiceCommand::ServiceStopPost as usize]){
                    Ok(_) => {},
                    Err(e) => return Err(e),
                }
            }
...
fn prepare_command(commands: &Vec<String>, command_list: &mut LinkedList<Rc<RefCell<CommandLine>>>) -> Result<(), Box<dyn Error>> {
    if commands.len() == 0 {
        return Ok(());
    }

    for exec in commands.iter() {
        let mut cmd: Vec<String> = exec.trim_end().split_whitespace().map(|s| s.to_string()).collect();
        if cmd.is_empty() {
            return Ok(());
        }
        ...
        let new_command = Rc::new(RefCell::new(CommandLine{cmd: path.to_str().unwrap().to_string(), args: cmd, next: None}));
